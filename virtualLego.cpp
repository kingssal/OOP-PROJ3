...
#include <vector>
#include <ctime> //시간과 날짜 관련 라이브러리
#include <cstdlib> //c의 표준 라이브러리 stdlib.h의 기능. 주로 메모리 할당, 난수 생성, 환경 제어, 정수 변환
#include <cstdio> //c의 stdio기능. 입출력 기능
#include <cassert> //프로그램의 특정 조건을 검증하는 메크로 제공, 주로 디버깅에 사용

IDirect3DDevice9* Device = NULL; //Direct3D 9 그래픽 장치 인터페이스에 대한 포인터를 선언하고 초기화
//IDirect3DDevice9는  Direct3D 9 API의 핵심 클래스. 개발자는 GPU에 그래픽 명령을 보내고, 텍스처를 관리하며, 렌더링 상태를 설정

// window size
const int Width  = 1024;  //가로
const int Height = 768;   //세로의 윈도우 사이즈
...
//하나의 공이 있다고 하자
//공의 위치를 초기화
const float spherePos[1][2] = { {0,-1.0f}}; //공의 초기 위치
conts D3DXCOLOR sphereColor[1] = {d3d::WHITE}; //공의 색상


// -----------------------------------------------------------------------------
// Transform matrices
// -----------------------------------------------------------------------------
D3DXMATRIX g_mWorld; //월드 변환 행렬, 객체의 위치, 회전, 크기를 월드 공간 좌표로 변활 할 때 사용한다.
D3DXMATRIX g_mView; //뷰 변환 행렬, 카메라의 위치와 방향을 정의
D3DXMATRIX g_mProj; // 투영 변환 행렬, 3D 공간의 객체를 2D 화면에 표시하기 위해 사용된다. 일반적으로 원근 투영이나 직교 투영을 정의 할 때 사용한다.

#define M_RADIUS 0.21   // 공의 반지름을 정의한다.
#define PI 3.14159265 //원주율
#define M_HEIGHT 0.01  //높이?
#define DECREASE_RATE 0.9982 //공의 속도 감소율

// ------------------------------------------------------------------------------------------
// CSphere class definition, 3D공을 표현하기 위한 클래스, 공의 위치, 재질, 속도, 렌더링과 관련된 정보를 관리한다
// ------------------------------------------------------------------------------------------

class CSphere {
private :
	float					center_x, center_y, center_z;   //공의 중심 좌표들
    float                   m_radius;                       //반지름
	float					m_velocity_x;                   //x축의 속도
	float					m_velocity_z;                   //z축의 속도

public:
    CSphere(void)
    {
        D3DXMatrixIdentity(&m_mLocal);                      //로컬변환행렬을 
        ZeroMemory(&m_mtrl, sizeof(m_mtrl));                //재질 구조체를 0으로 초기화
        m_radius = 0;
		m_velocity_x = 0;
		m_velocity_z = 0;
        m_pSphereMesh = NULL;
